function merge() {
  ...
  var data1Sheet = ss.getSheetByName("Bad quality"); // Получаем лист "Bad quality"
  ...
  var amount1Index1 = header1.indexOf("14D Deposit/TC");
  var amount2Index1 = header1.indexOf("1M Deposit/TC");
  var statusIndex = header1.indexOf("Fin status");

  var idIndex2 = header2.indexOf("Split 1");
  var dateIndex = header2.indexOf("Cohort Date");
  var amount1Index2 = header2.indexOf("14D Deposit OAS");
  var amount2Index2 = header2.indexOf("1M Deposit OAS");

  // Проверка: все ли нужные столбцы найдены
  if ([idIndex1, periodIndex, amount1Index1, amount2Index1, idIndex2, dateIndex, amount1Index2, amount2Index2, statusIndex].includes(-1)) {
  ...
  for (var i = 1; i < data2.length; i++) {
    ...
    var amount1 = raw1 ? Math.round(parseFloat(raw1) * 100) + "%" : ""; // Преобразуем в целое значение %
    var amount2 = raw2 ? Math.round(parseFloat(raw2) * 100) + "%" : "";

    if (!dataMap[id]) dataMap[id] = {}; // Если ID ещё нет, создаём
    dataMap[id][month] = [amount1, amount2]; // Сохраняем данные по ID и месяцу
  }
  ...
  for (var i = 1; i < data1.length; i++) {
    ...
    if (months.length === 1) {
      // Одномесячный период
      var month = months[0];
      if (dataMap[id] && dataMap[id][month]) {
      ...
      // Считаем средние значения по всем месяцам
      var sum1 = 0, sum2 = 0, count1 = 0, count2 = 0;
      for (var m = 0; m < allsortedMonths.length; m++) {
        var mon = allsortedMonths[m];
        var val1 = parseFloat(dataMap[id][mon][0].replace("%", ""));
        var val2 = parseFloat(dataMap[id][mon][1].replace("%", ""));
        if (!isNaN(val1)) { sum1 += val1; count1++; }
        if (!isNaN(val2)) { sum2 += val2; count2++; }
      }

      if (count1 || count2) {
        // Считаем среднее и записываем в ячейки
        var avg1 = count1 ? Math.round((sum1 / count1).toFixed(2)) + "%" : "";
        var avg2 = count2 ? Math.round((sum2 / count2).toFixed(2)) + "%" : "";

        data1Sheet.getRange(i + 1, amount1Index1 + 1).setValue(avg1);
        data1Sheet.getRange(i + 1, amount2Index1 + 1).setValue(avg2);

        generatedCompositePeriods[id][compositeKey] = [avg1, avg2]; // Сохраняем, чтобы не пересчитывать
      }
    }
  }
  ...
  var outputData = [["Split 1", "Cohort Date", "14D Deposit OAS", "1M Deposit OAS"]]; // Заголовок
